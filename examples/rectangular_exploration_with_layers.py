    ...
    with SyncCrazyflie(URI, cf=cf) as scf:
        ...
        with MotionCommander(scf, 0.5) as motion_commander:
            with Multiranger(scf) as multiranger:
                # 3 different layers
                starting_Low = True
                away_From_Wall = False
                layer_Cleared = False
                keep_flying = True
                # the 4 different ranges for the multiranger deck
                ...
                while (keep_flying):
                    ...
                    while not first_wall_is_found:
                        ...
                        if is_close(multiranger.front):
                            ...
                            if multiranger.right < multiranger.left and right_distance < 1:  # 1 is arbitrary, need to change the value
                                ...
                                if multiranger.right == (
                                        right_distance * math.sin(angle_to_yaw_radians) + 0.1) or multiranger.right == (
                                        right_distance * math.sin(angle_to_yaw) - 0.1):
                            ...
                            # if there is an obstacle closer to the drone on the left side than on the left side
                                ...
                                # check whether the drone is // or not by checking the right distance between the drone and the wall
                                if multiranger.left == (
                                        left_distance * math.sin(angle_to_yaw_radians) + 0.1) or multiranger.left == (
                                        left_distance * math.sin(angle_to_yaw) - 0.1):
                                    is_parallel_to_first_wall = True
                    ...
                    while not target_is_found():
                        ...
                        #it will rotate when reaches an end only if that is not the end of the layer
                        if is_close(multiranger.front) and not layer_Cleared:
                            motion_commander.stop()

                            if switch == "next_is_left" and multiranger.left > multiranger.right and not is_close(
                                    multiranger.left):
                                motion_commander.turn_left(90)
                                time.sleep(0.1)
                                if not is_close(multiranger.front):
                            ...
                            elif switch == "next_is_right" and multiranger.right > multiranger.left and not is_close(
                                    multiranger.right):
                                motion_commander.turn_right(90)
                                time.sleep(0.1)
                                if not is_close(multiranger.front):
                                ...
                                away_From_Wall = True
                        #if it reaches the end and it is the of the layer, it will go up 1 layer
                        elif is_close(multiranger.front) and layer_Cleared:
                            time.sleep(0.1)
                            if starting_Low:
                                motion_commander.up(height_step)
                            else:
                                motion_commander.down(height_step)
                            layer_Cleared = False
                            away_From_Wall= False

                        #if it is not close to anything, it will continue going forward
                        if not is_close(multiranger.front):
                            motion_commander.start_forward(VELOCITY)
                            if away_From_Wall:
                                if is_close(multiranger.right) or is_close(multiranger.left):
                                    layer_Cleared = True;
